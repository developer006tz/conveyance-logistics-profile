  ...
  const certificates = [
    {
      ...
      image: "/legal/business-license.png",
    },
    {
      title: "Certificate of Incorporation",
      image: "/legal/certificate-of-incorporations.png",
    },
    {
      title: "Business License",
      image: "/legal/tax-clearance.png",
    },
    {
      title: "Tin certificate",
      image: "/legal/tin-certificate.png",
    },
  ];

  return (
    <>
      <PageNavigation currentPath="/statutory-documents" />
      <div className="w-[210mm] min-h-[297mm] mx-auto bg-secondary relative overflow-hidden print:w-[210mm] print:h-[297mm] print:overflow-hidden print:break-after-page">
        ...
        <div className="relative z-10 flex flex-col h-full p-16 print:p-16">
          ...
          {Array.from({ length: Math.ceil(certificates.length / 2) }).map((_, rowIndex) => (
            <div key={rowIndex} className="bg-white/5 backdrop-blur-sm rounded-lg pr-4 py-2 pl-2 mb-6 shadow-xl flex gap-8">
              {/* First certificate in the pair */}
              <div className="w-[50%]">
                <img 
                  src={certificates[rowIndex * 2].image} 
                  alt={certificates[rowIndex * 2].title} 
                  className="w-full h-full object-fill rounded-lg"
                />
              </div>
              {/* Second certificate in the pair (if it exists) */}
              <div className="w-[50%]">
                {rowIndex * 2 + 1 < certificates.length && (
                  <img 
                    src={certificates[rowIndex * 2 + 1].image} 
                    alt={certificates[rowIndex * 2 + 1].title} 
                    className="w-full h-full object-fill rounded-lg"
                  />
                )}
              </div>
